 ...
 */
public class DBcone12 extends HttpServlet {
  protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try  {
            ...
            db_st=db_con.prepareStatement("select * from profiles where name like=? tell=? birth=?");
            db_st.executeUpdate();
            
            
            db_st.setString(1, "%"+name1+"%");
            db_st.setString(2, tell1);
            db_st.setString(3, birth1);
            
            db_data = db_st.executeQuery();
            
            
            while(db_data.next()){
                out.print("名前は"+db_data.getString("name1"));
                out.print("電話番号は"+db_data.getString("tell1"));
                out.print("誕生日は"+db_data.getString("birth1"));
            }
